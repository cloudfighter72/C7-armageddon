...
usage() {
  cat <<EOF
Usage:
  REGION=us-east-2 INSTANCE_ID=i-0dd6447b2f3ab96b8 SECRET_ID=db-secret ./scripts/gate_secrets_role.sh
...
Examples:
  ...
  REQUIRE_ROTATION=true REGION=us-east-2 INSTANCE_ID=i-0dd6447b2f3ab96b8 SECRET_ID=db-secret ./scripts/gate_secrets_role.sh

  # Run on the EC2 and verify it can read the secret value (does NOT print it):
  CHECK_SECRET_VALUE_READ=true REGION=us-east-2 INSTANCE_ID=i-0dd6447b2f3ab96b8 SECRET_ID=db-secret ./scripts/gate_secrets_role.sh
EOF
}
